...
//JavaScript ООП, классы (здесь мы заполняем карточки сериалов)

const DBService = class {

  constructor() {
    this.SERVER = 'https://api.themoviedb.org/3';
    this.API_KEY = 'a12cf5c4d448a96877da0d0b4a1204cc';
  }
...
const renderCard = (response, target) => {
  ...
  if (!response.total_results) {
    ...
    tvShowsHead.style.cssText = 'color: #ff637f;';
    return;
  }

  tvShowsHead.textContent = target ? target.textContent : 'Результат поиска:';
  tvShowsHead.style.cssText = 'color: #00838f;';
...
leftMenu.addEventListener('click', (event) => {
  ...
  if (target.closest('#search')) {
    tvShowsList.textContent = '';
    tvShowsHead.textContent = 'Введите поисковый запрос:';
